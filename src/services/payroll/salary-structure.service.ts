...
import { calculateNetPay, officeQueryGenerator} from '@/utils/payrollUtil';
...
class SalaryStructureService {
  ...
  public async create(data: ISalaryStructure): Promise<any> {
    try {
      ...
      const netPay = calculateNetPay(componentsData);
      data.netPay = netPay;
  ...
  public async update(id:string, data: IUpdateSalaryStructure): Promise<any>{
    try {
      const records = []
      const projection = {amount:1, type:1}
      let netPay;
      ...
      if(records.length > 0)
      {
        ...
        netPay = calculateNetPay(componentsData);
      }
      if(netPay)
      {
        data.netPay = Number(netPay);
      }
