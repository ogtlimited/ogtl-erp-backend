...
class JobApplicantController {
  ...
  public getJobApplicants = async (req, res:Response, next:NextFunction) =>{
    try {
      const applicants: { jobApplicants: IJobApplicant[]; pagination: IJobApplicantPagination } = await this.jobApplicantService.getJobApplicants(req.user._id,req.query)
